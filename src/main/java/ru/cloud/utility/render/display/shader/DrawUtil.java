package ru.cloud.utility.render.display.shader;
...
import ru.cloud.utility.interfaces.IWindow;
...
import ru.cloud.utility.render.display.base.CustomSprite;
...
public class DrawUtil implements IWindow {
    ...
    private GlProgram glassProgram;
    ...
    public void initializeShaders() {
        ...
        glassProgram = new GlProgram(Zenith.id("liquidglass/data"), VertexFormats.POSITION_TEXTURE_COLOR);
    ...
    public void drawLiquidGlass(MatrixStack matrices,
                                float x,
                                float y,
                                float width,
                                float height,
        ...
        glassProgram.findUniform("Radius").set(
                borderRadius.topLeftRadius(),
                borderRadius.bottomLeftRadius(),
                borderRadius.topRightRadius(),
                borderRadius.bottomRightRadius()
        );
        glassProgram.findUniform("Smoothness").set(DEFAULT_SMOOTHNESS);
        glassProgram.findUniform("FresnelPower").set(fresnelPower);
        glassProgram.findUniform("FresnelColor").set(
                fresnelColor.getRed() / 255.0f,
                fresnelColor.getGreen() / 255.0f,
                fresnelColor.getBlue() / 255.0f
        );
        glassProgram.findUniform("FresnelAlpha").set(fresnelColor.getAlpha() / 255.0f);
    ...
    public boolean isLiquidGlassStyle() {
        return Zenith.getInstance().getThemeManager().getGuiStyle() == GuiStyle.LIQUID_GLASS;
    }
    ...
    public void drawImageAlpha(MatrixStack matrices, Identifier identifier, float x, float y, float width, float height, ColorRGBA color1, ColorRGBA color2, ColorRGBA color3, ColorRGBA color4) {
        matrices.push();
