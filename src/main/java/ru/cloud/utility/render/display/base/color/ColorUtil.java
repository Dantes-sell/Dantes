package ru.cloud.utility.render.display.base.color;
...
public class ColorUtil {
    ...
    public int makeColor(int red, int green, int blue, int alpha) {
        return ((MathHelper.clamp(alpha, 0, 255) << 24) |
                (MathHelper.clamp(red, 0, 255) << 16) |
                (MathHelper.clamp(green, 0, 255) << 8) |
                MathHelper.clamp(blue, 0, 255));
    ...
    public int blend(int color1, int color2, float ratio) {
        float inv = 1f - ratio;
        int r = (int) (red(color1) * inv   + red(color2) * ratio);
        int g = (int) (green(color1) * inv + green(color2) * ratio);
        int b = (int) (blue(color1) * inv  + blue(color2) * ratio);
        int a = (int) (alpha(color1) * inv + alpha(color2) * ratio);
