package ru.cloud.utility.render.level;
...
public class Render3DUtil implements IMinecraft {
    ...
    public final List<Line> GLOW_LINE_DEPTH = new ArrayList<>();
    public final List<Line> GLOW_LINE = new ArrayList<>();
    public final List<Quad> QUAD_DEPTH = new ArrayList<>();
    public final List<Quad> QUAD = new ArrayList<>();
    public final List<Texture> GLOW_TEXTURE_DEPTH = new ArrayList<>();
    public final List<Texture> GLOW_TEXTURE = new ArrayList<>();
    ...
    public void onEventRender3D(MatrixStack matrix) {
        ...
        if (!GLOW_LINE.isEmpty()) {
            GL11.glEnable(GL11.GL_LINE_SMOOTH);
            Set<Float> widths = GLOW_LINE.stream().map(line -> line.width).collect(Collectors.toCollection(LinkedHashSet::new));
            RenderSystem.enableBlend();
            RenderSystem.disableCull();
            RenderSystem.disableDepthTest();
            ...
            widths.forEach(width -> {
                RenderSystem.lineWidth(width);
                BufferBuilder buffer = tessellator.begin(VertexFormat.DrawMode.LINES, VertexFormats.LINES);
                GLOW_LINE.stream().filter(line -> line.width == width).forEach(line -> vertexLine(matrix, buffer, line.start, line.end, line.colorStart, line.colorEnd));
                BufferRenderer.drawWithGlobalProgram(buffer.end());
            });
        ...
        if (!GLOW_LINE_DEPTH.isEmpty()) {
            GL11.glEnable(GL11.GL_LINE_SMOOTH);
            Set<Float> widths = GLOW_LINE_DEPTH.stream().map(line -> line.width).collect(Collectors.toCollection(LinkedHashSet::new));
            RenderSystem.enableBlend();
            RenderSystem.disableCull();
            RenderSystem.enableDepthTest();
            RenderSystem.depthMask(false);
            ...
            widths.forEach(width -> {
                RenderSystem.lineWidth(width);
                BufferBuilder buffer = tessellator.begin(VertexFormat.DrawMode.LINES, VertexFormats.LINES);
                GLOW_LINE_DEPTH.stream().filter(line -> line.width == width).forEach(line -> vertexLine(matrix, buffer, line.start, line.end, line.colorStart, line.colorEnd));
                BufferRenderer.drawWithGlobalProgram(buffer.end());
            });
            RenderSystem.blendFunc(GlStateManager.SrcFactor.SRC_ALPHA, GlStateManager.DstFactor.ONE_MINUS_SRC_ALPHA);
            RenderSystem.depthMask(true);
            RenderSystem.enableCull();
            RenderSystem.disableBlend();
            GLOW_LINE_DEPTH.clear();
            GL11.glDisable(GL11.GL_LINE_SMOOTH);
        }
        if (!GLOW_TEXTURE.isEmpty()) {
            Set<Identifier> ids = GLOW_TEXTURE.stream().map(t -> t.id).collect(Collectors.toCollection(LinkedHashSet::new));
            RenderSystem.enableBlend();
            RenderSystem.disableDepthTest();
            RenderSystem.blendFunc(GlStateManager.SrcFactor.SRC_ALPHA, GlStateManager.DstFactor.ONE);
            ids.forEach(id -> {
                ...
                GLOW_TEXTURE.stream().filter(t -> t.id.equals(id)).forEach(t -> quadTexture(t.entry, buffer, t.x, t.y, t.width, t.height, t.color));
                BufferRenderer.drawWithGlobalProgram(buffer.end());
            });
            RenderSystem.blendFunc(GlStateManager.SrcFactor.SRC_ALPHA, GlStateManager.DstFactor.ONE_MINUS_SRC_ALPHA);
            RenderSystem.enableDepthTest();
        ...
        if (!GLOW_TEXTURE_DEPTH.isEmpty()) {
            Set<Identifier> ids = GLOW_TEXTURE_DEPTH.stream().map(t -> t.id).collect(Collectors.toCollection(LinkedHashSet::new));
            RenderSystem.enableBlend();
            RenderSystem.enableDepthTest();
            RenderSystem.depthMask(false);
            RenderSystem.blendFunc(GlStateManager.SrcFactor.SRC_ALPHA, GlStateManager.DstFactor.ONE);
            ids.forEach(id -> {
                ...
                RenderSystem.setShader(ShaderProgramKeys.POSITION_TEX_COLOR);
                BufferBuilder buffer = tessellator.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_TEXTURE_COLOR);
                GLOW_TEXTURE_DEPTH.stream().filter(t -> t.id.equals(id)).forEach(t -> quadTexture(t.entry, buffer, t.x, t.y, t.width, t.height, t.color));
                BufferRenderer.drawWithGlobalProgram(buffer.end());
            });
            RenderSystem.blendFunc(GlStateManager.SrcFactor.SRC_ALPHA, GlStateManager.DstFactor.ONE_MINUS_SRC_ALPHA);
            RenderSystem.depthMask(true);
            RenderSystem.disableBlend();
            GLOW_TEXTURE_DEPTH.clear();
        }
    }
    ...
    public void quadTexture(MatrixStack.Entry entry, BufferBuilder buffer, float x, float y, float width, float height, Vector4i color) {
        if (entry == null) entry = new MatrixStack().peek();
        buffer.vertex(entry, x,         y + height, 0).texture(0, 1).color(color.x);
        buffer.vertex(entry, x + width, y + height, 0).texture(1, 1).color(color.y);
        buffer.vertex(entry, x + width, y,          0).texture(1, 0).color(color.w);
        buffer.vertex(entry, x,         y,          0).texture(0, 0).color(color.z);
    }

    public void drawGlowLine(Vec3d start, Vec3d end, int color, float width, boolean depth) {        drawGlowLine(start, end, color, color, width, depth);
    }

    public void drawGlowLine(Vec3d start, Vec3d end, int colorStart, int colorEnd, float width, boolean depth) {
        Vec3d cameraPos = mc.getEntityRenderDispatcher().camera.getPos();
        Line line = new Line(start.subtract(cameraPos), end.subtract(cameraPos), colorStart, colorEnd, width);
        if (depth) GLOW_LINE_DEPTH.add(line);
        else GLOW_LINE.add(line);
    }

    public void drawGlowTexture(MatrixStack.Entry entry, Identifier id, float x, float y, float width, float height, Vector4i color, boolean depth) {
        Texture texture = new Texture(entry, id, x, y, width, height, color);
        if (depth) GLOW_TEXTURE_DEPTH.add(texture);
        else GLOW_TEXTURE.add(texture);
    }
