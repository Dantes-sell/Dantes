...
@ModuleAnnotation(name = "Strafe", category = Category.MOVEMENT, description = "????????? ?????? Strafe")
public final class Strafe extends Module {
    ...
    private final ModeSetting.Value modeDefault = new ModeSetting.Value(mode, "???????").select();
    ...
    private final BooleanSetting dmgBoost   = new BooleanSetting("???? ?? ?????", "???? ?? ?????", false);
    private final NumberSetting  boostSpeed = new NumberSetting("???????? ?????", 0.7f, 0.1f, 5.0f, 0.1f, dmgBoost::isEnabled);
    private final NumberSetting  boostDuration = new NumberSetting("???????????? ?????", 500f, 100f, 2000f, 50f, dmgBoost::isEnabled);
