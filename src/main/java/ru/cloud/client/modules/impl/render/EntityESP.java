...
@ModuleAnnotation(name = "EntityESP", category = Category.RENDER, description = "Есп для ентити")
    ...
    private void renderNameTag(CustomDrawContext context, PlayerEntity player, float tickDelta) {
        ...
        if ( this.elements.isEnable(2)) {
            ...
            if (nonEmpty > 0) {
                ...
                float rectX = centerX - totalWidth / 2;
                ...
                for (ItemStack stack : stacks) {
                    if (!stack.isEmpty()) {
                        context.pushMatrix();
                        context.getMatrices().translate(centerX - totalWidth / 2 + currentOffset + 1 * scale.getCurrent(), rectY, 0);
