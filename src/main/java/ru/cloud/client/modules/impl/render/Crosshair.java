...
// ������ ��������
@ModuleAnnotation(name = "Crosshair", category = Category.RENDER, description = "????????? ?????? Crosshair")
public final class Crosshair extends Module {
    ...
    private final NumberSetting thickness = new NumberSetting("???????", 1.0f, 0.5f, 3.0f, 0.1f);
    private final NumberSetting length = new NumberSetting("?????", 3.0f, 1.0f, 8.0f, 0.5f);
    private final NumberSetting gap = new NumberSetting("??????", 2.0f, 0.0f, 5.0f, 0.5f);
    private final BooleanSetting dynamicGap = new BooleanSetting("???????????? ??????", false);

    private final BooleanSetting useEntityColor = new BooleanSetting("???? ????????", false);
